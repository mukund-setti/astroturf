...
export default async function ClusterDetailPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      <SiteHeader backHref={`/dockets/${docketId}`} backLabel={`Docket ${docketId}`} />
      ...
      <main className="flex-1 bg-background text-foreground">
        ...
        <div className="bg-card border-b border-rule py-3">
          <div className="mx-auto max-w-6xl px-6 flex items-center gap-2 text-[10px] md:text-xs uppercase tracking-wider text-muted-foreground flex-wrap">
            {topicObj && (
              <>
                <Link href={`/topics/${topicObj.id}`} className="hover:text-brand transition-colors font-medium">
                  {topicObj.name}
                </Link>
                <span>→</span>
              </>
            ...
            {agencyObj && (
              <>
                <Link href={`/agencies/${agencyObj.id}`} className="hover:text-brand transition-colors font-medium">
                  {agencyObj.name}
                </Link>
                <span>→</span>
              </>
            ...
            {docketObj && (
              <Link href={`/dockets/${docketObj.id}`} className="hover:text-brand transition-colors font-medium text-foreground">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed mb-4">
                    ...
                    one-hour window (**19:00 - 19:59 UTC**). This temporal clustering indicates a bulk API injection 
