...
export default async function DocketDetailPage({ params }: PageProps) {
            ...
            <section className="mx-auto max-w-4xl px-6 py-12 md:py-16 text-center space-y-6">
              ...
              <div className="max-w-xl mx-auto p-6 border border-rule bg-card rounded-sm space-y-4">
                ...
                <div className="pt-2 flex flex-wrap justify-center gap-3">
                  <DocketRefreshButton />
                  <Link
                    href={`/legacy/analysis/${matchedReq.request_id}`}
  ...
  if (docket.status === "ingestion_ready" && !isFromDatabase && !matchedReq) {
    return (
      <>
        <SiteHeader backHref="/legacy/topics" backLabel="Analyzed coverage" />
        <main className="flex-1 bg-background text-foreground pb-20">
          <section className="mx-auto max-w-4xl px-6 py-12 md:py-16">
            ...
            <Link
              href={`/legacy/analyze?docket=${encodeURIComponent(docket.id)}&agency=${docket.agencyId}&topic=${docket.topicId}`}
  ...
  return (
    <>
      <SiteHeader backHref={`/legacy/topics/${docket.topicId}`} backLabel={topic?.name ?? "Sector"} />
        ...
        <section className="border-b border-rule bg-secondary/10 py-12 md:py-16">
          <div className="mx-auto max-w-6xl px-6">
            <div className="flex items-center gap-3 flex-wrap mb-4">
              <Link href={`/legacy/agencies/${docket.agencyId}`} className="text-[10px] font-sans uppercase tracking-wider text-brand bg-brand/10 px-2 py-0.5 rounded-sm font-semibold hover:bg-brand/20 transition-colors">
                {agency?.name ?? docket.agencyId}
              </Link>
              <Link href={`/legacy/topics/${docket.topicId}`} className="text-[10px] font-sans uppercase tracking-wider bg-secondary text-muted-foreground px-2 py-0.5 rounded-sm font-medium hover:bg-muted transition-colors">
