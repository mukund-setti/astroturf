"""Delta-rs helpers for gold.campaign_attributions writes.

Idempotent upserts keyed by ``attribution_id`` (see ADR-0015 for the primary
key construction). Mirrors the pattern in ``shared/delta_utils/gold.py``.
"""
...
def merge_campaign_attributions(
    path: str | Path,
    arrow_table: pa.Table,
) -> dict[str, int]:
    """Idempotent upsert into gold.campaign_attributions by attribution_id."""
    with local_tmp_delta_path(path) as local_path:
        ...
        metrics = (
            dt.merge(
                source=arrow_table,
                predicate="target.attribution_id = source.attribution_id",
                source_alias="source",
                target_alias="target",
            )
...
def delete_attribution_scope(
    ...
    *,
    ...
    """Delete prior attribution rows for a docket (optionally narrowed to clusters)."""
    with local_tmp_delta_path(path) as local_path:
